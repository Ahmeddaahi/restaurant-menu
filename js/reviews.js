...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.openReviewForm = function(val = 0) {
    ...
    async function loadReviews() {
        try {
            ...
            totalReviewsCount.textContent = `${visibleReviews.length} reviews`;
            
            // Update summary stars
            avgStarsSummary.innerHTML = Array(5).fill(0).map((_, i) => `
                <svg class="w-3 h-3 ${i < Math.round(avg) ? 'fill-current' : 'text-gray-200'}" viewBox="0 0 24 24" fill="currentColor">
            ...
            if (visibleReviews.length === 0) {
                reviewsList.innerHTML = '<p class="text-center text-gray-400 italic py-10">No reviews yet.</p>';
                return;
            }
